...
export class LotsService {
  ...
  async createLot(caseId: string, dto: CreateLotDto, actorId: string) {
    return this.prisma.$transaction(async (tx) => {
      ...
      // 7. COD-63: REUSE Lot에 자동 Listing 생성 (DRAFT)
      if (grading.routingDecision === RoutingDecision.REUSE) {
        const rule = await tx.settlementRule.findFirst({
          where: { partType: dto.partType, isActive: true },
          orderBy: { version: 'desc' },
        });
        if (rule) {
          const gradeBonusMap = (rule.gradeBonusMap as Record<string, number>) ?? {};
          const gradeBonus = grading.reuseGrade ? (gradeBonusMap[grading.reuseGrade] ?? 0) : 0;
          const autoPrice = rule.m0BaseAmount.toNumber()
            * (1 + rule.deltaRatio.toNumber() / 100)
            * (1 + gradeBonus / 100);

          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          await (tx.listing.create as any)({
            data: {
              lotId: lot.id,
              type: 'FIXED_PRICE',
